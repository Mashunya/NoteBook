...
public class AddProgramNameDecorator implements Command {
    ...
    public ModelAndView execute() {
        ...
        if(globalParams == null) {
            resultModelAndView.getModel().addMessage(new Message("Глобальные параметры не установлены", Message.WARNING));
        ...
        if(programName != null && version != null) {
            ...
            resultModelAndView.getModel().addMessage(new Message(aboutProgram, Message.INFO));
        } else {
            resultModelAndView.getModel().addMessage(new Message("Глобальные параметры Program_Name и (или) " +
                    "Version не заданы", Message.WARNING));
        }
